...
public class CourseCutServiceImpl extends ServiceImpl<CourseCutDao, CourseCut> implements CourseCutService {
    ...
    public Result insertCourseCut(Long courseId, Long userId) {
        ...
        // 判断短剧是否已砍
        List<CourseCut> courseCutList = this.list(new QueryWrapper<CourseCut>().eq("course_id", courseId).eq("user_id", userId).ne("cut_status", 3));
        ...
        if (this.save(courseCut)) {
            CourseCut cut = this.getOne(new QueryWrapper<CourseCut>().eq("course_id", courseId).eq("user_id", userId).eq("cut_status", 0));
